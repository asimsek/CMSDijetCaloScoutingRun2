...
def initializeWorkspace(w,cfg,box,scaleFactor=1.,penalty=False,multi=True,x=None,emptyHist1D=None):
...
def writeDataCardMC(box,model,txtfileName,bkgs,paramNames,w):
       ...
       # processes.extend(["%s"%(bkg) for bkg in bkgs])
        if '2015' in box:
                ...
                lumiErrs.extend([1.025 for bkg in bkgs]) ## https://twiki.cern.ch/twiki/bin/viewauth/CMS/TWikiLUM?rev=167
        divider = "------------------------------------------------------------\n"
        datacard = "imax 1 number of channels\n" + \
                   "jmax %i number of processes minus 1\n"%(nBkgd+signals-1) + \
                   "kmax * number of nuisance parameters\n" + \
...
#            if f.lower().find('FourjetShape')!=-1:
    ...
    for name in names:
		...
		print "signalHistos", signalHistos[0]
    ...
    if options.inputFitFile is not None:
        ...
        if options.deco or options.refit:
            ...
            if 'p1_%s'%box in paramsToDecoNames:
                paramsToDeco.add(w.var('cop1_%s'%box))
    ...
    for shape in shapes:
        if shapeFiles[shape+'Up'] is not None:
            ...
            if options.trigger:
                ...
                #  hUp = applyTurnonFunc(fUp.Get('h_dat'),effFrIn,w)
            else:
    ...
    outFile = 'dijet_combine_%s_%i_lumi-%.3f_%s.root'%(model,massPoint,lumi,box)
