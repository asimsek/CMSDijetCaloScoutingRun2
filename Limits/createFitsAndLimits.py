...
def execute_commands(args, rMax, signalType, configFile, date, year, lumi, config, inputmjj, scaled):
    mass = "489"
    lumi2 = lumi / 1000
    xsecSignal = 10.0
    ...
    if not args.bf:
        ...
        subprocess.run([
            ...
            "--rMax", str(rMax), "--xsec", str(xsecSignal), "-l", str(lumi2), "--yr", year, "--scaled"
        ])
...
if __name__ == '__main__':
    ...
    if not args.scaled:
        for line in lines:
            ...
            execute_commands(args, float(rMax), signalType, configFile, date, year, float(lumi), config, str(inputmjj1), args.scaled)
    else:
        cfgFileTrue = configFile if not args.cfgFile else args.cfgFile
        execute_commands(args, float(args.rMax), args.signalType, cfgFileTrue, args.date, args.year, float(args.lumi), args.config, str(args.inputmjj), args.scaled)
