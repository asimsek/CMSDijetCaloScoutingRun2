...
def main():
    ...
    parser.add_argument("--mass", type=int, help="mass to analyze", default=0)
    parser.add_argument('--condor', action='store_true', default=False)
    parser.add_argument('--noAsk', action='store_true', default=False)
    ...
    for mass in massRange:
        val1 = get_expected_value(args.year, args.sig, filtered_df1['rMax'].values[0], filtered_df1['config'].values[0], filtered_df1['date'].values[0], mass, filtered_df1['configFile'].values[0])
        val2 = get_expected_value(args.year, args.sig, filtered_df2['rMax'].values[0], filtered_df2['config'].values[0], filtered_df2['date'].values[0], mass, filtered_df2['configFile'].values[0])
        expSigVal = int(args.muTrue) * val1
        expSigVal2 = int(args.muTrue) * val2
        ...
        if args.oppositeFuncs:
            commandlineGenerateToys_CMS_Only = "combine %s -M GenerateOnly -t %d --expectSignal %0.3f --cminDefaultMinimizerStrategy 0 --saveToys -n %s" % (inputDataCard2, int(args.toys), expSigVal2, nTag2)
        ...
        if args.sanityCheck:
            commandlineFitDiagnostic = "combine %s -M FitDiagnostics -t %d --saveToys --rMin %.2f --rMax %.2f --expectSignal %0.3f -n %s_rMax%.2f --cminDefaultMinimizerStrategy 0 --saveWorkspace --saveWithUncertainties --robustFit=1 --setRobustFitTolerance=1.0" % (inputDataCard, int(args.toys), rMin_, rMax_, expSigVal, nTag1, float(filtered_df1['rMax'].values[0]))

            commandlinePlotterOpposite = "python MaxLikelihood_Bias_Plotter.py --signal %s --year %s --mass %s --muTrue %.6f --cfgFile %s --Type Sanity_muTrue%s  --rMax %.2f --inputRoot fitDiagnostics%s_rMax%.2f.root" % (args.sig, args.year, mass, float(expSigVal), filtered_df1['configFile'].values[0], args.muTrue, rMax_, nTag1, float(filtered_df1['rMax'].values[0]))
    ...
    typeText = "--oppositeFuncs" if args.oppositeFuncs else "--sanityCheck" if args.sanityCheck else ""
    summaryPlot = "python biasSummaryPlotter.py {0} --muTrue {1} --year {2} --signalType {3}".format(typeText, args.muTrue, args.year, args.sig)
    print (summaryPlot)
    os.system(summaryPlot)
