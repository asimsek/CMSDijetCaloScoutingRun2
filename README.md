...
```sh
python biasResults.py --inputFile inputFiles/allRunIILimits_cfg_ModExp_4Param.txt --inputFile2 inputFiles/allRunIILimits_cfg.txt --muTrue 1 --year 2016B --sig qq --toys 1000 --oppositeFuncs
```
...
### Produce Summary Plotters between 800-1700 GeV

```sh
python biasSummaryPlotter.py --muTrue 1 --year 2016B --signalType qq --oppositeFuncs
```

### Produce bias results on Condor

```sh
python condorBiasResults.py --muTrue 1 --year 2016B --sig qq --toys 1000 --oppositeFuncs
```

If you need to generate and fit with the same function (`--inputFile`), use `--sanityCheck` instead of `--oppositeFuncs`.
