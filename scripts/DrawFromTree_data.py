...
allCuts_Data = "%s && %s && passHLT_CaloScoutingHT250==1" % (baseCut, jetIDCuts)
allCuts_MC = "%s && %s" % (baseCut, jetIDCuts)
...
for f in fileNames:
  ...
  tree.Project(h_dat.GetName(), var, allCuts_Data)
...
for f in fileNames_MC:
  ...
  tree.Project(h_MC.GetName(), var, allCuts_MC)
